...
async def main():
    ...
    async def generate_mindmap_simple(self, document_content: str, request_id: str) -> str:
        ...
        try:
            ...
            simple_prompt = f"""
            请根据以下文档内容，直接生成一个完整的思维导图结构。要求：

            1. 识别3-6个主要主题
            2. 每个主题下包含2-4个子主题
            3. 每个子主题下包含2-3个关键详细信息
            4. 返回JSON格式，包含完整的层级结构
            ...
            请以以下JSON格式返回思维导图结构：
            {{
                "central_theme": "Document Mindmap",
                "topics": [
                    {{
                        "name": "主题名称",
                        "emoji": "🔥",
            ...
            try:
                ...
                mindmap_structure = json.loads(cleaned_response)
                
                # Validate structure
                if not isinstance(mindmap_structure, dict) or 'topics' not in mindmap_structure:
                    raise ValueError("Invalid mindmap structure returned")
                
                # Convert to internal format
                concepts = {
                    'central_theme': {
                        'name': mindmap_structure.get('central_theme', 'Document Mindmap'),
                        'subtopics': []
                    }
                }
                
                # Process topics
                for topic_data in mindmap_structure.get('topics', []):
                    topic = {
                    ...
                    for subtopic_data in topic_data.get('subtopics', []):
                        subtopic = {
                            ...
                            'emoji': subtopic_data.get('emoji', '📋'),
                            'importance': subtopic_data.get('importance', 'medium'),
                            'details': []
                        }
                        ...
                        for detail_data in subtopic_data.get('details', []):
                            detail = {
                                ...
                                'importance': detail_data.get('importance', 'medium')
                            }
